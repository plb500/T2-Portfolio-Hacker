...
void click_and_display(const char *displayString) {
...
void pin_scan(const char *finalPIN) {
    ...
    do {
        ...
        click_and_display(displayBuffer);
    ...
    fputs("\nPIN IDENTIFICATION NUMBER: ", stdout);
    puts(finalPIN);
}
